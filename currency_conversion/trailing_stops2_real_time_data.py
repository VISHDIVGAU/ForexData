    ...
    def aggregate_raw_data_tables(self):
        with self.engine.begin() as conn:
            for i, curr in enumerate(self.currency_pairs): 
                ...
                conn.execute(text("INSERT INTO "+curr[0]+curr[1]+"_model_performance(predicted_return, actual_return , error) VALUES (:predicted_return, :actual_return , :error);"),[{"predicted_return":predicted_return, "actual_return": ret, "error": error}])
