...
class TrailingStopsData2:
    ...
    # this functions create table for each currency pair to store min and max ranges of fd and volatility after being sorted using method 1 i.e. sorting fd and volatility independently.
    def initialize_sort_one_tables(self):
        with self.engine.begin() as conn:
            for curr in self.currency_pairs:
                conn.execute(text("CREATE TABLE "+curr[0]+curr[1]+"_sort_one(fd_high_min  numeric, fd_high_max  numeric, fd_medium_min  numeric, fd_medium_max  numeric, fd_low_min  numeric, fd_low_max  numeric, vol_high_min  numeric, vol_high_max  numeric, vol_medium_min  numeric, vol_medium_max  numeric, vol_low_min  numeric, vol_low_max  numeric);"))
    
    # this functions create table for each currency pair to store min and max ranges of fd and volatility after being sorted using method 2 i.e. sorting fd first and then volatility .
    def initialize_sort_two_tables(self):
        with self.engine.begin() as conn:
            for curr in self.currency_pairs:
                conn.execute(text("CREATE TABLE "+curr[0]+curr[1]+"_sort_two(fd_high_min  numeric, fd_high_max  numeric, fd_medium_min  numeric, fd_medium_max  numeric, fd_low_min  numeric, fd_low_max  numeric, high_vol_high_min  numeric, high_vol_high_max  numeric, high_vol_medium_min  numeric, high_vol_medium_max  numeric, high_vol_low_min  numeric, high_vol_low_max  numeric, medium_vol_high_min  numeric, medium_vol_high_max  numeric, medium_vol_medium_min  numeric, medium_vol_medium_max  numeric, medium_vol_low_min  numeric, medium_vol_low_max  numeric, low_vol_high_min  numeric, low_vol_high_max  numeric, low_vol_medium_min  numeric, low_vol_medium_max  numeric, low_vol_low_min  numeric, low_vol_low_max  numeric);"))

    # this functions create table for each currency pair to store min and max ranges of fd and volatility after being sorted using method 3 i.e. sorting volatility first and then fd .
    def initialize_sort_three_tables(self):
        with self.engine.begin() as conn:
            for curr in self.currency_pairs:
                conn.execute(text("CREATE TABLE "+curr[0]+curr[1]+"_sort_three(vol_high_min  numeric, vol_high_max  numeric, vol_medium_min  numeric, vol_medium_max  numeric, vol_low_min  numeric, vol_low_max  numeric, high_fd_high_min  numeric, high_fd_high_max  numeric, high_fd_medium_min  numeric, high_fd_medium_max  numeric, high_fd_low_min  numeric, high_fd_low_max  numeric, medium_fd_high_min  numeric, medium_fd_high_max  numeric, medium_fd_medium_min  numeric, medium_fd_medium_max  numeric, medium_fd_low_min  numeric, medium_fd_low_max  numeric, low_fd_high_min  numeric, low_fd_high_max  numeric, low_fd_medium_min  numeric, low_fd_medium_max  numeric, low_fd_low_min  numeric, low_fd_low_max  numeric);"))

    # this function creates table to store best sorting method for all the currency pairs
    def initialize_best_classification_method(self):
        with self.engine.begin() as conn:
                conn.execute(text("CREATE TABLE best_classification(best_sort numeric);"))
    """
    ...
    def sorting_one(self):
        self.initialize_sort_one_tables()
        with self.engine.begin() as conn:
            for curr in self.currency_pairs:
                df_fd= pd.read_sql_query("SELECT fractal_dimension FROM "+curr[0]+curr[1]+"_agg;", self.engine) # creating data frame with fractal_dimension coloumn
                df_vol= pd.read_sql_query("SELECT volatility FROM "+curr[0]+curr[1]+"_agg;", self.engine) # creating data frame with volatility coloumn
                ...
                conn.execute(text("INSERT INTO "+curr[0]+curr[1]+"_sort_one (fd_high_min, fd_high_max, fd_medium_min, fd_medium_max, fd_low_min, fd_low_max, vol_high_min, vol_high_max, vol_medium_min, vol_medium_max, vol_low_min, vol_low_max) VALUES (:fd_high_min, :fd_high_max, :fd_medium_min, :fd_medium_max, :fd_low_min, :fd_low_max, :vol_high_min, :vol_high_max, :vol_medium_min, :vol_medium_max, :vol_low_min, :vol_low_max);"),[{"fd_high_min":fd_high_min, "fd_high_max":fd_high_max, "fd_medium_min":fd_medium_min, "fd_medium_max":fd_medium_max, "fd_low_min":fd_low_min, "fd_low_max":fd_low_max, "vol_high_min":vol_high_min, "vol_high_max":vol_high_max, "vol_medium_min":vol_medium_min, "vol_medium_max":vol_medium_max, "vol_low_min":vol_low_min, "vol_low_max":vol_low_max}])
    ...
    def sorting_two(self):
        self.initialize_sort_two_tables()
        with self.engine.begin() as conn:
            for curr in self.currency_pairs:
                df = pd.read_sql_query("SELECT fractal_dimension, volatility FROM "+curr[0]+curr[1]+"_agg;", self.engine) # creating dataframe with coloumn fractal_dimension, volatility
    ...
    def sorting_three(self):
        self.initialize_sort_three_tables()
        with self.engine.begin() as conn:
            for curr in self.currency_pairs:
                df = pd.read_sql_query("SELECT fractal_dimension, volatility FROM "+curr[0]+curr[1]+"_agg;", self.engine) # create data frame with coloumns fractal_dimension, volatility
    ...
    def clasify_one(self, curr, i):
        with self.engine.begin() as conn:
            df= pd.read_sql_query("SELECT avgfxrate, fractal_dimension, volatility, return*1000000 as return FROM "+curr[0]+curr[1]+"_agg;", self.engine)
    ...
    def train_model(self):
        ...
        for index, df in enumerate(self.dfs):
            ...
            with self.engine.begin() as conn:
                conn.execute(text("INSERT INTO best_classification(best_sort) VALUES (:best_sort);"),[{"best_sort": ind+1}]) # saving value in table. This will be use in real time predictions
            if ind==0:
