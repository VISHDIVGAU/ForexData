...
class TrailingStopsDataProject:
    """
    ...
    def __init__(self):
        self.currency_pairs = [ ["EUR","USD",{'upper': list(), 'lower': list()},0,100,
                                ...
                                ["USD","EUR",{'upper': list(), 'lower': list()},0,100,
                                {'sorting_one':{'fractal_dimension':{'high':[],'medium':[],'low':[]},'volatility':{'high':[],'medium':[],'low':[]}},
                                'sorting_two':{'fractal_dimension':{'high':{'range':[], 'volatility':{'high':[],'medium':[],'low':[]}}, 'medium':{'range':[], 'volatility':{'high':[],'medium':[],'low':[]}},'low':{'range':[], 'volatility':{'high':[],'medium':[],'low':[]}}}},
                                'sorting_three':{'volatility':{'high':{'range':[], 'fractal_dimension':{'high':[],'medium':[],'low':[]}}, 'medium':{'range':[], 'fractal_dimension':{'high':[],'medium':[],'low':[]}}, 'low':{'range':[], 'fractal_dimension':{'high':[],'medium':[],'low':[]}}}}
                                    }],
                                ["USD","GBP",{'upper': list(), 'lower': list()},0,100,
                                {'sorting_one':{'fractal_dimension':{'high':[],'medium':[],'low':[]},'volatility':{'high':[],'medium':[],'low':[]}},
                                'sorting_two':{'fractal_dimension':{'high':{'range':[], 'volatility':{'high':[],'medium':[],'low':[]}}, 'medium':{'range':[], 'volatility':{'high':[],'medium':[],'low':[]}},'low':{'range':[], 'volatility':{'high':[],'medium':[],'low':[]}}}},
                                'sorting_three':{'volatility':{'high':{'range':[], 'fractal_dimension':{'high':[],'medium':[],'low':[]}}, 'medium':{'range':[], 'fractal_dimension':{'high':[],'medium':[],'low':[]}}, 'low':{'range':[], 'fractal_dimension':{'high':[],'medium':[],'low':[]}}}}
                                    }],
    ...
    def sorting_one(self):
        for curr in self.currency_pairs:
            ...
            index_range= df_vol[ df_vol['volatility'] == 0 ].index
            #print(index_range)
            df_vol=df_vol.drop(index_range)
            df_fd= df_fd.drop(index_range)
            df_vol['volatility']= df_vol['volatility'].multiply(10000)
            df_fd['fractal_dimension']= df_fd['fractal_dimension'].divide(1000000)
            ...
            hfd= df_fd.iloc[:33,:].agg({'fractal_dimension':['min', 'max']}) # first 33 elements in FD
            hvol= df_vol.iloc[:33,:].agg({'volatility':['min', 'max']}) # first 33 elements in volatility
            curr[5]['sorting_one']['fractal_dimension']['high'].extend(hfd['fractal_dimension'].tolist()) # saving ranges in dictionary for this currency in currency pair list
            curr[5]['sorting_one']['volatility']['high'].extend(hvol['volatility'].tolist()) #saving ranges in dictionary for this currency in currency pair list
            mfd= df_fd.iloc[33:67,:].agg({'fractal_dimension':['min', 'max']}) # next 32 elements in fd
            mvol= df_vol.iloc[33:67,:].agg({'volatility':['min', 'max']}) # next 32 elemenst in Volatility
            curr[5]['sorting_one']['fractal_dimension']['medium'].extend(mfd['fractal_dimension'].tolist()) 
            curr[5]['sorting_one']['volatility']['medium'].extend(mvol['volatility'].tolist()) 
            lfd= df_fd.iloc[67:,:].agg({'fractal_dimension':['min', 'max']}) # next 33 elements of fd
            lvol=df_vol.iloc[67:,:].agg({'volatility':['min', 'max']}) # next 33 elements of volatility
            curr[5]['sorting_one']['fractal_dimension']['low'].extend(lfd['fractal_dimension'].tolist())
            curr[5]['sorting_one']['volatility']['low'].extend(lvol['volatility'].tolist())
            # variable to be use in storing data in database
            fd_high_min= curr[5]['sorting_one']['fractal_dimension']['high'][0]
            fd_high_max = curr[5]['sorting_one']['fractal_dimension']['high'][1]
            fd_medium_min= curr[5]['sorting_one']['fractal_dimension']['medium'][0]
            fd_medium_max= curr[5]['sorting_one']['fractal_dimension']['medium'][1]
            fd_low_min= curr[5]['sorting_one']['fractal_dimension']['low'][0]
            fd_low_max= curr[5]['sorting_one']['fractal_dimension']['low'][1]
            vol_high_min= curr[5]['sorting_one']['volatility']['high'][0]
            vol_high_max= curr[5]['sorting_one']['volatility']['high'][1]
            vol_medium_min= curr[5]['sorting_one']['volatility']['medium'][0]
            vol_medium_max= curr[5]['sorting_one']['volatility']['medium'][1]
            vol_low_min= curr[5]['sorting_one']['volatility']['low'][0]
            vol_low_max= curr[5]['sorting_one']['volatility']['low'][1]
    ...
    def sorting_two(self):
        ...
        for curr in self.currency_pairs:
            ...
            index_range= df[ df['volatility'] == 0 ].index
            df=df.drop(index_range)
    ...
    def clasify_one(self, curr, i):
        agg_coll= self.db[curr[0]+curr[1]+"_agg"]
        df= pd.DataFrame(list(agg_coll.aggregate([{"$project": {"_id":0 ,"inserttime":1,"avgfxrate":1,"fractal_dimension":1,"volatility":1,"return":1}}])))
        index_range= df[ df['volatility'] == 0 ].index
        df=df.drop(index_range)
        ...
        df['minute']= pd.to_datetime(df['inserttime']).dt.minute
        df.drop(['inserttime'], axis=1, inplace=True)
        df= df[['day','hour','minute', 'avgfxrate', 'fractal_dimension','volatility','return']]
    ...
    def train_model(self):
        for index, df in enumerate(self.dfs):
            ...
            #mask_one= df[0]['day'] <16
            #one_train= df[0][mask_one] # one_train.shape= (157,8)
            #mask_one= df[0]['day']==16
            #one_test= df[0][mask_one] # one_test.shape= (118,8)
            #one_train.drop(['day','hour'], axis=1, inplace=True)
            #one_test.drop(['day','hour'], axis=1, inplace=True)
            #reg1= pycr.setup(data= one_train, test_data= one_test, target= 'return', numeric_features=['day','hour','minute','avgfxrate'],categorical_features=['fractal_dimension','volatility'])
            reg1= pycr.setup(data= df[0],target= 'return', numeric_features=['day','hour','minute','avgfxrate'],categorical_features=['fractal_dimension','volatility'])
            best1= pycr.compare_models(sort="RMSLE")
            model1= pycr.create_model(best1)
            ...
            pred1= pycr.predict_model(final_model1)
            print(pred1)
            #mask_two= df[1]['day'] <16
            #two_train= df[1][mask_two] # one_train.shape= (157,8)
            #mask_two= df[1]['day']==16
            #two_test= df[1][mask_two] # one_test.shape= (118,8)
            #two_train.drop(['day','hour'], axis=1, inplace=True)
            #two_test.drop(['day','hour'], axis=1, inplace=True)
            #reg2= pycr.setup(data= two_train, test_data= two_test, target= 'return', numeric_features=['day','hour','minute','avgfxrate'],categorical_features=['fractal_dimension','volatility'])
            reg2= pycr.setup(data= df[1],target= 'return', numeric_features=['day','hour','minute','avgfxrate'],categorical_features=['fractal_dimension','volatility'])
            best2= pycr.compare_models(sort='RMSLE')
            model2= pycr.create_model(best2)
            t_model2= pycr.tune_model(model2) # we will fine tune default hyperparameters of the model
            final_model2= pycr.finalize_model(t_model2) # train the model on whole data set
            pred2= pycr.predict_model(final_model2)
            print(pred2)
            #mask_three= df[2]['day'] <16
            #three_train= df[2][mask_three] # one_train.shape= (157,8)
            #mask_three= df[2]['day']==16
            #three_test= df[1][mask_three] # one_test.shape= (118,8)
            #three_train.drop(['day','hour'], axis=1, inplace=True)
            #three_test.drop(['day','hour'], axis=1, inplace=True)
            #reg3= pycr.setup(data= three_train, test_data= three_test, target= 'return', numeric_features=['day','hour','minute','avgfxrate'],categorical_features=['fractal_dimension','volatility'])
            reg3= pycr.setup(data= df[2], target= 'return', numeric_features=['day','hour','minute','avgfxrate'],categorical_features=['fractal_dimension','volatility'])
            best3= pycr.compare_models(sort='RMSLE')
            model3= pycr.create_model(best3)
            t_model3= pycr.tune_model(model3) # we will fine tune default hyperparameters of the model
            final_model3= pycr.finalize_model(t_model3) # train the model on whole data set
            pred3= pycr.predict_model(final_model3)
            print(pred3)
            l= list()
            l.append(pycu.check_metric(pred1['return'], pred1['Label'], 'R2'))  # we will get the mean R2 value of the predictions of this model
            l.append(pycu.check_metric(pred2['return'], pred2['Label'], 'R2'))
            l.append(pycu.check_metric(pred3['return'], pred3['Label'], 'R2'))
            print(l)
